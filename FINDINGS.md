...
|---|------|-----|-----|-------------|--------|
| 1 | 🔴 | `backend/controllers/orderController.js::addOrderItems` | `return` после `throw` — недостижимый код; излишняя вложенность `if/else` | Заменить `if/else` на guard clause: `if (!orderItems \|\| !orderItems.length) { throw ... }` | 🔴 not yet |
